...
REM ════════════════════════════════════════════════════════════════════════════════════════
REM Enhanced Batch Wrapper for Radarr Rename Script
REM 
REM Configuration note: In config.env, paths with spaces can be specified with or without quotes:
REM   GIT_BASH_PATH=C:\Program Files\Git\bin\bash.exe          (recommended)
REM   GIT_BASH_PATH="C:\Program Files\Git\bin\bash.exe"        (also works)
REM   
REM This script automatically handles both formats and ensures proper quoting.
REM ════════════════════════════════════════════════════════════════════════════════════════
            ...
            if errorlevel 1 (
                ...
                if not errorlevel 1 (
                    for /f "tokens=1,2 delims==" %%b in ("!line!") do (
                        set "var_name=%%b"
                        set "var_value=%%c"
                        REM Remove quotes from variable value if present
...
REM Clean up any remaining quotes from paths (safety measure)
...
echo [%date% %time%] DEBUG: GIT_BASH_PATH="%GIT_BASH_PATH%" (after quote cleanup) >> "%LOG_FILE%"
echo [%date% %time%] DEBUG: RENAME_SH_PATH="%RENAME_SH_PATH%" (after quote cleanup) >> "%LOG_FILE%"
